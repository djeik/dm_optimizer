...
def plotf_3d(f, xyzs_, start=np.array([-1,-1]), end=np.array([1,1]),
    ...
    """ Plot a function in three dimensions and show a trajectory on it.
        ...
        Arguments:
            ...
            xyzs_ (ndarray):
                the trajectory to draw, in the format [(z, (x, y)].
            start (ndarray):
                a corner in the bounding box of the sampled domain.
            end (ndarray):
                the opposite corner in the bounding box.
            smoothness (float):
                how sparsely is the domain sampled. Smaller values look better,
                but take longer to render.
            autobound (bool):
                automatically determine the bounding box based on the
                trajectory if True.
            autosmooth (bool):
                automatically calculate a decent smoothing value, based on the
                bounding box.

        Returns:
            The generated figure, so that it may be rendered to a window or
            saved to a file by the caller.
        """
    fig = plt.figure()
...
def multiplot(dats, names=[], nrows=None, ncols=1):
    """ Make multiple plots in the case where each x value has several y values
        associated with it.  If nrows is None, then the number of rows is
        calculated based on the size of the tuples in dats and the number of
        columns specified. """
...
def calculate_stats(test, rs, time_total):
    ...
    success_total = len(filter(
        lambda r: r.success
                  and abs(optimization_type * r.fun - test["optimum"]) < \
                      experiment_defaults["success_threshold"],
                               rs))
...
def calculate_averages(statistics): # [[[a]]] -> [[a]]
    """ Take a [[[a]]], where [a] is the period sampling of a datum each
        iteration, [[a]] is such a sampling done on many individual runs, and
        finally [[[a]]] is a list of such statistics, and produce a [[a]] the
        same length as the input [[[a]]] that is a a list of averages w.r.t.
        time.
        """
    avgs = []
    ...
    # For the performance. Zero looks like it's giving SA an advantage.
    avgs[-2][-1] *= len(filter(lambda x: x == 0, avgs[-2]))
    return avgs
    ...
    for (name, average_vs_t, data) in complete_experiment:
        with open(exp_dir + "/" + name + ".txt", 'w') as f:
            for (i, run_i) in enumerate(data):
                f.write(str(average_vs_t[i]) if i < len(average_vs_t)
                        else str(None))
                for value in run_i:
                    f.write(',' + str(value))
                f.write('\n')
...
def experiment_task(args):
    ...
    (failures, success_rate, time_avg, nfev_avg, rs) = \
            conduct_experiment(exp_dir, test, optimizer);
    end_time = time()
    j.errprint(test["name"] + ":",
            end_time - start_time, "seconds spent performing experiment.")
    ...
    if is_dm(optimizer):
        ...
        data_vs_iter  = [[[run[iteration_i][data_i]
                           if iteration_i < len(run) else None
                           for run in vs]
...
def conduct_all_experiments_inner(edir, optimizer, names=poll_names,
    ...
    global_averages = tuple(map(
        lambda stat: sum(stat) / float(len(stat)),
        zip(*all_statistics)))
    global_stdevs   = tuple(map(np.std, zip(*all_statistics)))

    return (results, all_statistics, global_averages, global_stdevs)

# statistics measured: success rate, average runtime, average function evals,
# function value vs time, best minimum vs time, stepsize vs time
def conduct_all_experiments(edir, optimizer,
        experiment_defaults=experiment_defaults, names=poll_names):
    # we will collect the general statistics for each experiment here, to
    # perform a global average over all experiments.
    all_statistics = []
    global_statistics = []
    ...
    start_time = time()
    results, all_statistics, global_averages, global_stdevs = \
            conduct_all_experiments_inner(edir, optimizer, names)
    end_time = time()

    with open(path.join(edir, "averages.txt"), 'w', 1) as f:
        print_csv("test", "success rate", "average time",
                "average fun. evals.", "average performance", "failures",
                file=f)
        # print the general test data to the common file
        map(lambda (name,
            (success_rate, time_avg, nfev_avg, perf_avg, failures)): \
                    print_csv(name, success_rate, time_avg,
...
def parse_typical_poll_file(path):
    """ A poll file is one generated from write_experiment_data. The names of
        all the polls is in poll_names. """
    def sanitize(string):
        try:
            return float(string)
        except ValueError:
...
def generate_all_dm_plots(edir):
    ...
    function_dirs = filter(
            lambda p: path.isdir(path.join(dmdir, p)),
            os.listdir(dmdir)) # function_dirs will be relative to dmdir

    if not function_dirs:
        raise ValueError(
                "fatal: the previous step did not produce any data to plot.")
        ...
        for poll in poll_names:
            ...
            for individual_run in zip(*individuals):
                ...
                ax.plot(xrange(1, len(ys) + 1), ys,
                        color=plots_config["individual_color"])
...
def solved_vs_iterations_inner(solver_dir, optimizer_name, test,
    ...
    """ Run the given optimizer on the given test, to generate data for a
        "fraction of successful runs versus time" plot.

        Arguments:
            solver_dir (string):
                the directory in which to create the data/ and results/
                directories, where runtime information about the solver and the
                resulting <function>.csv file are saved, respectively. Usually,
                `solver_dir` is something like "results/<date & time>/<solver
                name>".
            optimizer_name (string):
                the name of the solver to use. This is used as a key in the
    ...
    # data_points is just a list of ints, that say how long it took for that
    # run to finish
    def alive_vs_t(lifetimes):
        ...
        for i in xrange(iterations_config["end"]):
            ...
            # record how many runs were still going at this iteration
            alives.append(len(ls))
        return alives
...
def parse_solved_vs_iterations_data_for_one_optimizer(data_dir, runs_count):
    """ Return a dict associating each objective function in
        ...
        Arguments:
            data_dir (string):
                the path to the folder that contains the <function>.csv files.
            runs_count (int):
                the number of runs to average over.

        Notes:
            The <function>.csv files should be the number of runs _still
            operating_ at the iteration number identified by the line number in
            the file. This function will take care of performing the
            calculation
                (a - x) / a
            where a is the total number of runs (runs_count) and x is the value
            on that line.
        """
    ...
    d = dict(zipmap(lambda test_name: with_file(
            ...
            # for each test name we get the path to the data file
...
def solved_vs_iterations_plots(data_dir,
    ...
    for (func_name, each_optimizer) in data.items():
        ...
        fig.savefig(path.join(
            data_dir, "solved_vs_iterations", func_name + ".pdf"))
